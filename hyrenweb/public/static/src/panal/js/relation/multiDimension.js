;(function(echarts, window) {
	/**
	   多维图表构造器
	*/
	function MulDimensionChart(dom,data,dimension,measure,bcolor) {
		this.finalData = data;
		...
		this.dom = dom;
		this.colors = [bcolor, "#fa5879", "#f8e402", "#4feaef", "#9d58fa", "#5899fa", "#58fa6b", "#fa6b58"];
		this.init();
	}
	MulDimensionChart.prototype = {
		...
		getDimensionCategory: function(data, dLen, mLen, isContinue) {
			...
			if(dLen == 1){
				for(var i in chartData){
					var data = chartData[i].data[0];
					var dataArray = [];
					for(var j in data){
						dataArray.push(data[j][1]);
					}
					chartData[i].data[0] = dataArray;
				}
			}else{
				if (dLen >= 2) {
		...
		*/
		...
		standarHigherData: function (data, measure, dimension) {
			...
			function addAxisDataItem(groupedCategoryData, dimension, chartData, index, dataIndex, context) {
				...
				var quantityGloab = context.maxPerBarQuantity;
				var quantity = gridData.count;
				
				if(typeof(quantityGloab) == 'undefined' || quantityGloab < quantity){
					context.maxPerBarQuantity = quantity;
				}
		...
		*/
        getXAxisOption: function(params) {
		    ...
		    var xAxisObj = {
				...
				mulDimension: true,
				axisLabel: {
					show: function (i, measureIndex, mLen, dLen, categoryIndex) {
						if (mLen === 0) {
					...
					}(index, measureIndex, mLen, params.dimension.length, params.categoryIndex),
		...
		*/
        getSeriesOption: function(params) {
			...
			var barSpecialWidth = Math.floor(params.perChartWidth / context.maxPerBarQuantity) * 0.4;
		    var measureIndex = params.measureIndex;
	  	...
	  	*/
	  	processTooltipFormatter: function(dimension, measure, disContinuousKey, params) {
	  		...
	  		if (params.componentType == 'markLine') {//处理参考线的显示悬浮层
				...
				//表示X轴只有单个维度，数据格式为[1,2,3]，该处data为其中一项
				if(typeof(data) == "number"){
					formatter = getFormatterBysingel(color, params.seriesName, params.name, data);
				}else{
	        ...
	        return formatter;
	  		...
	  		*/
	  		function getFormatter(color, name, value) {
	  		     return  ('<span style="display:inline-block;display:inline-block;border-radius:10px;width:9px;height:9px;background-color:'
			...
			/**
				X轴单个维度时的html字符串
			*/
			function getFormatterBysingel(color, title, name, value) {
	  		     return  ('<span style="border-radius:10px;width:9px;height:9px;background-color:'
	  			           + color + '"></span>' + title + '</br>'
						   +'<span style="display:inline-block;display:inline-block;border-radius:10px;width:9px;height:9px;background-color:'
	  			           + color + '"></span>' + name +'：' + value + '</br>'
						   );
	  		}
	  	},
