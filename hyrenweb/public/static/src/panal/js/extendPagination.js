/**
  * 提供分页功能
**/
  function callBackPagination(totalCount,limit,callBackPager,pageTableList) {
		var showCount = 10;
        createTable(1, limit, totalCount,pageTableList);
        $("#"+callBackPager+"").extendPagination({
            totalCount: totalCount,
            showCount: showCount,
            limit: limit,
            callback: function (curr, limit, totalCount) {
                createTable(curr, limit, totalCount,pageTableList);
            }
        });
    }
  function callBackPaginationDiv(totalCount,limit,callBackPager,pageTableList) {
		var showCount = 10;
		createTableDiv(1, limit, totalCount,pageTableList);
		$("#"+callBackPager+"").extendPagination({
          totalCount: totalCount,
          showCount: showCount,
          limit: limit,
          callback: function (curr, limit, totalCount) {
        	  createTableDiv(curr, limit, totalCount,pageTableList);
          }
      });
  }
    function createTable(currPage, limit, total,pageTableList) {
        var html = [], showNum = limit;
        if (total - (currPage * limit) < 0) showNum = total - ((currPage - 1) * limit);

		var beginPage = currPage <= 1 ? 0 : (currPage - 1) * limit;
		var endPage = beginPage + showNum;
		var aindex = new Array(showNum);
		aindex[0] = 0;
		for(i=beginPage+1;i<=endPage;i++){
			aindex[i] = i;
		}
		$("#"+pageTableList+"").find("tr").each(function(i){
			$(this).hide();
			if(i==aindex[i])$(this).show();
		});
    }

    function createTableDiv(currPage, limit, total,pageTableList) {
        var html = [], showNum = limit;
        if (total - (currPage * limit) < 0) showNum = total - ((currPage - 1) * limit);
		...
		$("#"+pageTableList+"").find(".smdiv").each(function(i){
			$(this).hide();
			if(i==aindex[i])$(this).show();
		});
    }
...
(function ($) {
    $.fn.extendPagination = function (options) {
        ...
        if (totalPage > 0) {
            var html = [];
            html.push(' <ul class="pagination" id="pagination">');
            html.push(' <li class="previous"><a href="#">&laquo;'+$.i18n.prop('system.page.shangyiye')+'</a></li>');
            ...
            html.push('<li class="disabled hidden"><a href="#">...</a></li>');
            html.push('<li class="next"><a href="#">'+$.i18n.prop('system.page.xiayiye')+'&raquo;</a></li></ul>');
            $(this).html(html.join(''));
