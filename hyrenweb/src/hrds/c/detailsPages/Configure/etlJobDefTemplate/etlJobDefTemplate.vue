<template>
  <div>
    ...
    <!-- 点击上传时模态框 -->
    <el-dialog title="作业程序参数" :visible.sync="dialogFormVisibleUpload" width="35%">
      <el-form :model="formUpload" ref="formUpload" class="demo-ruleForm" label-width="170px">
...
<script>
import * as etlJobDefTemplateAllFun from "./etlJobDefTemplate";
export default {
  data() {
    return {
      sys_cd:'',
  ...
  methods: {
    ...
    getInfo(){
      let params = {};
      etlJobDefTemplateAllFun.searchEtlJobTemplate(params).then(res=>{
    ...
    filesBtn(item,index){
      ...
      etlJobDefTemplateAllFun.searchEtlJobTempAndParam(params).then(res=>{
    ...
    getSelect(){
      ...
      etlJobDefTemplateAllFun.searchEtlSubSys(params).then(res=>{
        arr = res.data;
        arr.forEach((item)=>{
