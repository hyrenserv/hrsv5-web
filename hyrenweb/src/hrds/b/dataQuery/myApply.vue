...
<div>
    <el-row>
        <template v-for="item in ApplyType">
            <template v-if="apply_type === item.code">
                <span class="el-icon-s-management" :key="item.code" style="color: #3a8ee6; margin-left: 10px">
                    我的{{item.value}}列表
                </span>
            </template>
        </template>
    ...
    <el-row>
        ...
        <!-- 分页 -->
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[5, 10, 50, 100, 500]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="apply_rs.length" />
    </el-row>
</div>
</template>
