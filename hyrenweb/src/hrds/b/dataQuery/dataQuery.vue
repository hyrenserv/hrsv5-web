                    ...
                    <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal">
                        ...
                        <el-submenu index="3">
                            ...
                            <el-menu-item index="3-2"
                                          @click="fileClassificationQuery(recentData.officefile)">
                                {{recentData.officefile_zh}}
                            </el-menu-item>
                            <el-menu-item index="3-3"
                                          @click="fileClassificationQuery(recentData.wenbenfile)">
                                {{recentData.wenbenfile_zh}}
                            </el-menu-item>
                            <el-menu-item index="3-4"
                                          @click="fileClassificationQuery(recentData.yasuofile)">
                                {{recentData.yasuofile_zh}}
                            </el-menu-item>
                            <el-menu-item index="3-5"
                                          @click="fileClassificationQuery(recentData.rizhifile)">
                                {{recentData.rizhifile_zh}}
                            </el-menu-item>
                            <el-menu-item index="3-6"
                                          @click="fileClassificationQuery(recentData.biaoshujufile)">
                                ...
                                <el-row v-if="scope.row.is_others_apply === '0'">
                                    ...
                                    <el-col :span="8">
                                        <template v-if="scope.row.apply_type.indexOf(recentData.chakan) !== -1">
                                            <!-- 包含有查看 -->
                                            <a v-if="recentData.auth.includes(scope.row.auth_type.split(',')[scope.row.apply_type.split(',').indexOf(recentData.chakan)])"
                                        ...
                                        <template v-else>
                                            <a href="javascript:void(0)" class="linkStyle text-primary"
                                               @click="applyFor(scope.row.file_id,recentData.chakan,recentData.chakan_zh)">申请查看</a>
                                        </template>
                                    </el-col>
                                    <!-------------------------------- 下载 -------------------------------->
                                    <el-col :span=" 8">
                                        <template v-if="scope.row.apply_type.indexOf(recentData.xiazai) !== -1">
                                            <a v-if="recentData.auth.includes(scope.row.auth_type.split(',')[scope.row.apply_type.split(',').indexOf(recentData.xiazai)])"
                                               class="linkStyle text-success" href="javascript:void(0)"
                                        ...
                                        <template v-else>
                                            <a href="javascript:void(0)" class="linkStyle text-primary"
                                               @click="applyFor(scope.row.file_id,recentData.xiazai,recentData.xiazai_zh)">申请下载</a>
                                        </template>
                                    </el-col>
                                    <!-------------------------------- 发布 -------------------------------->
                                    <el-col :span="8">
                                        <template v-if="scope.row.apply_type.indexOf(recentData.fabu) !== -1">
                                            <a v-if="recentData.auth.includes(scope.row.auth_type.split(',')[scope.row.apply_type.split(',').indexOf(recentData.fabu)])"
                                        ...
                                        <template v-else>
                                            <a class="linkStyle text-primary" href="javascript:void(0)"
                                               @click="applyFor(scope.row.file_id,recentData.fabu,recentData.fabu_zh)">申请发布</a>
                                        </template>
                                    </el-col>
                                </el-row>
                                <el-row v-else>
    ...
    export default {
        ...
        data() {
            return {
                ...
                dataSourceList: [],
                taskList: [],
                conditionalQueryForm: {sourceId: '', fcsId: '', startDate: '', endDate: '',},
                recentData: [],
                recentFiles: [],
                fileClassifySum: [],
