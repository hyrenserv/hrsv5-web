        ...
        <el-col style="width: 70%;">
            <el-row class="bottomMargin">
                <el-col :span="5" :offset="1">
                    <el-button type="success" @click="routeToMyApply(apply_type_map.get('view').code)">
                        <el-row><span>
                ...
                <el-col :span="5" :offset="1">
                    <el-button type="primary" @click="routeToMyApply(apply_type_map.get('download').code)">
                        <el-row><span>
                ...
                <el-col :span="5" :offset="1">
                    <el-button type="warning" @click="routeToMyApply(apply_type_map.get('release').code)">
                        <el-row><span>
                ...
                <el-col :span="5" :offset="1">
                    <router-link to="myRequestRecord">
                        <el-button type="info" size="medium">
                            <el-row><span>
                                    <el-badge :value="myApplicationRecord" />次</span></el-row>
            ...
            <el-row class="borderStyle bottomMargin">
                <el-form :inline="true" ref="form" :model="queryForm" label-width="100px" size="medium">
                    <el-form-item label="数据源列表">
                        <el-select v-model="queryForm.sourceId" clearable placeholder="选择数据源" @change="getTaskIdByDataSourceId(queryForm.sourceId)" value="sourceId">
                            <el-option v-for="item in dataSourceList" :key="item.source_id" :label="item.datasource_name" :value="item.source_id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="任务列表">
                        <el-select v-model="queryForm.fcsId" clearable placeholder="选择任务" value="fcs_name">
                            <el-option v-for="item in taskList" :key="item.fcs_id" :label="item.fcs_name" :value="item.fcs_id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="采集日期">
                        <el-col :span="11">
                            <el-date-picker v-model="queryForm.startDate" type="date" placeholder="开始日期" value-format="yyyyMMdd" style="width: 100%;">
                            </el-date-picker>
                        </el-col>
                        <el-col class="line" :span="1" style="margin-left: 2%">--</el-col>
                        <el-col :span="11">
                            <el-date-picker v-model="queryForm.endDate" type="date" placeholder="结束日期" value-format="yyyyMMdd" style="width: 100%;">
                            </el-date-picker>
                        </el-col>
                    </el-form-item>
                    <el-form-item>
            ...
            <el-row class="borderStyle bottomMargin">
                <el-menu default-active="1001" class="el-menu-demo" mode="horizontal">
                    <el-menu-item v-for="i in file_type_list" :key="i.code" :index="i.code" @click="fileClassificationQuery(i.code)">
                        <template v-if="i.code === '1001'"><i class="el-icon-menu" />{{i.value}}</template>
                        <template v-else-if="i.code === '1002'"><i class="el-icon-picture" />{{i.value}}</template>
                        <template v-else-if="i.code === '1004'"><i class="el-icon-video-camera" />{{i.value}}</template>
                        <template v-else-if="i.code === '1005'"><i class="el-icon-service" />{{i.value}}</template>
                        <template v-else-if="i.code === '1006'"><i class="el-icon-plus" />{{i.value}}</template>
                        <template v-else>
                            <el-submenu index="1003">
                                <template slot="title"><i class="el-icon-document" />{{i.value}}</template>
                                <el-menu-item v-for="j in file_type_list_sub" :key="j.code" :index="j.code" @click="fileClassificationQuery(j.code)">{{j.value}}
                                </el-menu-item>
                            </el-submenu>
                        </template>
                    </el-menu-item>
                            ...
                            <el-row v-if="isOthersApply(scope.row.is_others_apply)">
                                ...
                                <el-col :span="8">
                                    <template v-if="isViewApply(scope.row.apply_type)">
                                        <!-- 包含有查看 -->
                                        <a v-if="isView(scope.row)" class="linkStyle text-success" href="javascript:void(0)" @click="viewFile(scope.row.file_id,scope.row.file_type)">查看</a>
                                        <a v-else class="linkStyle text-warning" href="javascript:void(0)">查看审核中</a>
                                    </template>
                                    <template v-else>
                                        <a href="javascript:void(0)" class="linkStyle text-primary" @click="applyFor(scope.row.file_id,apply_type_map.get('view').code,apply_type_map.get('view').value)">
                                            申请查看</a>
                                    </template>
                                </el-col>
                                <!-------------------------------- 下载 -------------------------------->
                                <el-col :span=" 8">
                                    <template v-if="isDownloadApply(scope.row.apply_type)">
                                        <a v-if="isDownload(scope.row)" class="linkStyle text-success" href="javascript:void(0)" @click="downloadFile(scope.row.file_id,scope.row.original_name)">下载</a>
                                        <a v-else class="linkStyle text-warning" href="javascript:void(0)">下载审核中</a>
                                    </template>
                                    <template v-else>
                                        <a href="javascript:void(0)" class="linkStyle text-primary" @click="applyFor(scope.row.file_id,apply_type_map.get('download').code,apply_type_map.get('download').value)">申请下载</a>
                                    </template>
                                </el-col>
                                <!-------------------------------- 发布 -------------------------------->
                                <el-col :span="8">
                                    <template v-if="isReleaseApply(scope.row.apply_type)">
                                        <a v-if="isRelease(scope.row)" class="linkStyle text-success" href="javascript:void(0)">已发布</a>
                                        <a v-else class="linkStyle text-warning" href="javascript:void(0)">发布审核中</a>
                                    </template>
                                    <template v-else>
                                        <a class="linkStyle text-primary" href="javascript:void(0)" @click="applyFor(scope.row.file_id,apply_type_map.get('release').code,apply_type_map.get('release').value)">申请发布</a>
                                    </template>
                                </el-col>
                            </el-row>
                            <el-row v-else>
                                <el-col :span="8">
                                    <a class="linkStyle text-primary" href="javascript:void(0)" @click="applyFor(scope.row.file_id,apply_type_map.get('view').code,apply_type_map.get('view').value)">申请查看</a>
                                </el-col>
                                <el-col :span="8">
                                    <a class="linkStyle text-primary" href="javascript:void(0)" @click="applyFor(scope.row.file_id,apply_type_map.get('download').code,apply_type_map.get('download').value)">申请下载</a>
                                </el-col>
                                <el-col :span="8">
...
export default {
    ...
    data() {
        return {
            ...
            dataSourceList: [],
            taskList: [],
            queryForm: { sourceId: '', fcsId: '', startDate: '', endDate: '', },
            recentData: [],
            recentFiles: [],
            fileClassifySum: [],
    ...
    mounted() {
        /* 初始化显示数据 */
        dataQuery.getConditionalQuery(this.queryForm).then((res) => {
    ...
    methods: {
        ...
        getAuthType() {
            this.$Code.getCategoryItems({
                ...
                res.data.forEach(row => {
                    ...
                    //处理为map
                    if (row.code === '1') map.set('YunXu', row);
                    else if (row.code === '2') map.set('BuYunXu', row);
                    else if (row.code === '3') map.set('YiCi', row);
                    else if (row.code === '0') map.set('ShenQing', row);
                });
        ...
        //是查看或者查看审核中
        isView(row) {
            return this.auth_type_map.get('YunXu').code === row.auth_type.split(',')[row.apply_type.split(',').indexOf(this.apply_type_map.get('view').code)] ||
                this.auth_type_map.get('YiCi').code === row.auth_type.split(',')[row.apply_type.split(',').indexOf(this.apply_type_map.get('view').code)];
        },
        //是否包含下载
        isDownloadApply(apply_type) {
            return apply_type.indexOf(this.apply_type_map.get('download').code) !== -1
        },
        //是下载或者下载审核中
        isDownload(row) {
            return this.auth_type_map.get('YunXu').code === row.auth_type.split(',')[row.apply_type.split(',').indexOf(this.apply_type_map.get('download').code)] ||
                this.auth_type_map.get('YiCi').code === row.auth_type.split(',')[row.apply_type.split(',').indexOf(this.apply_type_map.get('download').code)];
        },
        //是否包含发布
        ...
        //是发布或者发布审核中
        isRelease(row) {
            return this.auth_type_map.get('YunXu').code === row.auth_type.split(',')[row.apply_type.split(',').indexOf(this.apply_type_map.get('release').code)];
        }
    }
}
</script>
