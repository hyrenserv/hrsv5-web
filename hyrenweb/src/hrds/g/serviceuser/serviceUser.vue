...
<div class="interfaceInfo">
    ...
    <!--接口使用信息列表展示-->
    <el-table :data="interfaceUseData.slice((currPage - 1) * pageSize,currPage * pageSize)" border stripe style="width: 100%" size="medium">
        <el-table-column label="序号" width="50px" align="left">
            <template slot-scope="scope">
                <span>{{scope.$index+(currPage - 1) * pageSize + 1}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="interface_name" label="接口名称" align="left" />
        <el-table-column prop="start_use_date" :formatter="dateFormat" label="开始使用日期" align="left" />
        <el-table-column prop="use_valid_date" :formatter="dateFormat" label="结束使用日期" align="left" />
    ...
    <!--数据使用信息列表展示-->
    <el-table :data="dataUseData.slice((currDataPage - 1) * pageDataSize,currDataPage * pageDataSize)" border stripe style="width: 100%" size="medium">
        <el-table-column label="序号" align="left" width="50px">
            <template slot-scope="scope">
                <span>{{scope.$index+(currDataPage - 1) * pageDataSize + 1}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="sysreg_name" label="系统登记表名" align="left" />
        <el-table-column prop="original_name" label="原始表中文名" align="left" />
        <el-table-column label="表列信息查看" align="left">
            <template slot-scope="scope">
                <el-button size="mini" type="text" class='editcolor' @click="searchColumnInfoById(scope.row)">
                    表字段查看
                </el-button>
            </template>
        </el-table-column>
    ...
    <!--查看字段弹出框-->
    <el-dialog title="表列名信息" :visible.sync="dialogViewFieldFormVisible" :before-close="beforeViewFieldClose">
...
export default {
    ...
    methods: {
        // 查询数据使用信息
        searchDataTableInfo() {
            interfaceFunctionAll.searchDataTableInfo({
                "sysreg_name": this.sysreg_name
            }).then(res => {
        ...
        // 查询接口使用信息
        searchInterfaceInfo() {
            interfaceFunctionAll.searchInterfaceInfo({
        ...
        searchColumnInfoById(row) {
            ...
            interfaceFunctionAll.searchColumnInfoById({
                "use_id": row.use_id
            }).then(res => {
