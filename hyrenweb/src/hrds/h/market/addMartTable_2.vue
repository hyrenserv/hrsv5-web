    ...
    <div id="addMartable2">
        ...
        <el-row>
            <el-col class="borderStyle" :span="7" style="margin-right: 10px;height:50vh;overflow:auto">
                ...
                <div class='mytree'>
                    <el-tree class="filter-tree" :data="treedata" :indent='0' :show-checkbox="checktablenode()"
                             @node-click="showtablecolumn" :filter-node-method="filterNode" ref="tree">

                    <span class="span-ellipsis" slot-scope="{ node, data }">
                        <span :title="data.description"
                              v-if="'undefined' !== typeof data.file_id && data.file_id !== ''">
        ...
        <el-dialog title="关联表信息" :visible.sync="relationTableVisible" width="85%">
            <el-form :inline="true" class="demo-form-inline" :label-position="labelPosition"
                ...
                <el-row :gutter="10">
                    ...
                    <el-col v-for="(item,index) in relationTableList_1" :key="index" :span="24">
                        ...
                        <el-form-item label="关联表">
                            <el-input v-model="item.table_name" placeholder="关联表表名称">
                            </el-input>
                        </el-form-item>
                        <el-form-item>
                        ...
                        <el-form-item>
                            <el-input type="textarea" v-model="item.on_condition" placeholder="关联条件"/>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="danger" @click="deleteRelationTableList(index)" size="medium">删除
                            </el-button>
                        </el-form-item>
                    </el-col>
                    <el-col>
                        <el-form-item label="where">
                            <el-input placeholder="过滤条件,如果使用占位符，使用方式如：column=#{column}"
                                      v-model="relationTableAttr.whereColumns" style="width: 500px">
                            </el-input>
                        </el-form-item>
                    </el-col>
                    <el-col>
                        <el-form-item label="group by">
                            <el-input placeholder="分组条件" v-model="relationTableAttr.groupColumns">
                            </el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row style="text-align: right">
                    <el-form-item>
                        <el-button type="primary" size="medium" @click="relationTableVisible = false">
                            取消
                        </el-button>
                        <el-button type="primary" size="medium" @click="addSql2()">确定</el-button>
                    </el-form-item>
                </el-row>
            </el-form>
        </el-dialog>
        ...
        <el-dialog title="选择表" :visible.sync="selectTableVisible" width="80%">
            <el-row :gutter="10">
                ...
                <el-col :span='16' style="border-left: 1px #e0dcdc dashed;min-height: 600px;">
                    <el-table :data="tableData.slice((currPage - 1) * pageSize,currPage*pageSize)" border size="medium"
                              highlight-current-row ref="multipleTable" @current-change="handleSelectionChange"
                              @row-click="chooseone">
        ...
        <el-tabs type="card">
            <el-row>
                ...
                <el-col v-if="ifhbase" :span='2' style="float:right;margin-right:10px;width:360px;">
                    <el-input v-model="pre_partition" prop="pre_partition" placeholder="请输入预分区键，如 a,b,c 或者是预分区数，如 10"
                              clearable>
                    </el-input>
                </el-col>
                ...
                <el-table :data="columnbysql" border size="mini">
                    ...
                    <el-table-column prop="field_type" label="字段类型" width="130" show-overflow-tooltip align="center">
                        <template slot-scope="scope" width="120">
                            <el-select :disabled="iflock || scope.row.field_process  == '2'"
                                       v-model="scope.row.field_type" placeholder="请选择">
                    ...
                    <el-table-column prop="field_length" label="字段长度" width="140" align="center" class="fieldDesc">
                        ...
                        <template slot-scope="scope">
                            <el-input style="width:80%" :disabled="iflock || scope.row.field_process == '2'"
                                      v-model="scope.row.field_length" autocomplete="off" placeholder="长度"></el-input>
                    ...
                    <el-table-column prop="process_para" label="来源值" min-width="400" show-overflow-tooltip
                                     align="center">
                        <template slot-scope="scope">
                            <div v-if="scope.row.field_process != '5'">
                                ...
                                <el-input v-if="scope.row.field_process == '3'" readonly
                                          v-model="scope.row.process_mapping">
                                ...
                                <el-input v-if="scope.row.field_process == '4'" :title="scope.row.process_mapping"
                                          :clearable="true" placeholder="映射规则" v-model="scope.row.process_mapping">
                                    <el-button slot="append" @click="settingRule(scope.$index)">设置</el-button>
                                </el-input>
                            </div>
                            <div v-if="scope.row.field_process == '5'">
            ...
            <el-row>
                <el-table :data="databysql" border size="mini">
                    <el-table-column v-for="(index, item) in databysql[0]" :key="item" :label="item"
                                     show-overflow-tooltip :prop="item">
            ...
            <el-tabs v-model="activeName">
                <el-tab-pane v-for="item in tableDatalist.classify" :key="item" :label="item" :name="item">
                    <el-table stripe
                        ...
                        <el-table-column prop="function_name" label="函数名" show-overflow-tooltip
                                         align="left"></el-table-column>
                        <el-table-column prop="function_example" label="例子" align="left"
                                         show-overflow-tooltip></el-table-column>
                        <el-table-column prop="function_desc" label="描述" align="left"
                                         show-overflow-tooltip></el-table-column>
        ...
        methods: {
            ...
            getcolumnbysql() {
                ...
                if (this.basicInfoForm.sqlMain === '') {
                    ...
                    // };
                    functionAll.getColumnBySql(param).then(((res) => {
            ...
            getdatabysql() {
                ...
                if (this.basicInfoForm.sqlMain === '') {
                    ...
                    param.append("querysql", this.basicInfoForm.sqlMain);
                    param.append("sqlparameter", this.sqlparameter);
                    functionAll.getDataBySQL(param).then((res) => {
                        this.isLoading = false;
            ...
            next() {
                ...
                // params.append("datatable_field_info",JSON.stringify(this.columnbysql))
                // params.append("dm_column_storage",JSON.stringify(dm_column_storage))
                // params.append("hbasesort",JSON.stringify(this.hbasesort))
                // params.append("datatable_id",this.datatable_id);
                // params.append("querysql",this.basicInfoForm.sqlMain);
                // params.append("pre_partition",this.pre_partition);
                let param = {
