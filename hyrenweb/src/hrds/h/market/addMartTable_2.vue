    ...
    <div id="addMartable2">
            ...
            <el-row :gutter="10">
                ...
                <el-col :span='16' style="border-left: 1px #e0dcdc dashed;min-height: 600px;">
                    <el-table :data="tableData.slice((currPage - 1) * pageSize,currPage*pageSize)" border
                              size="medium" highlight-current-row ref="multipleTable"
        ...
        <el-tabs type="card">
            ...
            <el-row>
                <el-table :data="columnbysql" border size="mini">
                    ...
                    <el-table-column prop="field_type" label="字段类型" width="130" show-overflow-tooltip
                                     align="center">
                    ...
                    <el-table-column prop="field_length" label="字段长度" width="100" show-overflow-tooltip
                                     align="center">
                        <template slot-scope="scope">
                            <el-input :disabled="iflock" v-model="scope.row.field_length" autocomplete="off"
                                      placeholder="长度"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column prop="field_process" label="处理方式" width="130" show-overflow-tooltip
                                     align="center">
                        <template slot="header">
                            处理方式
                            <el-tooltip class="tooltipHelp" effect="dark"
                                        content="映射仅赋值：源表字段的值直接给模型表；映射且修改：源表的值通过写映射规则进行修改，改变后的值给模型表；分组映射：源表字段的值直接给模型表，同时映射规则填写模型表的字段=某个类别，如type=1的格式进行分组（横表转竖表时用）"
                                        placement="right">
                    ...
                    <el-table-column prop="process_para" label="来源值" min-width="400" show-overflow-tooltip
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.field_process != '5'">
                                ...
                                <el-input v-if="scope.row.field_process == '4'"
                                          :title="scope.row.process_mapping" :clearable="true"
                            ...
                            <div v-if="scope.row.field_process == '5'">
                                ...
                                <el-input style="width:50%" v-model="scope.row.group_mapping"
                                          autocomplete="off" placeholder="分组映射填写"></el-input>
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="field_desc" label="描述" width="180" show-overflow-tooltip
                                     align="center">
                        <template slot-scope="scope">
        ...
        <el-dialog title="规则列表" :visible.sync="ruleDialog" width="60%">
            <el-table stripe :data="tableDatalist" size="medium" border height="350">
                ...
                <el-table-column prop="function_example" label="例子" align="center"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column prop="function_desc" label="描述" align="center"
                                 show-overflow-tooltip></el-table-column>
