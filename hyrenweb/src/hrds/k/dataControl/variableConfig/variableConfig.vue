...
<div id='dataControl'>
    ...
    <el-row>
        ...
        <!-- 分页 -->
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 50, 100, 500]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" style="text-align: center" :total="var_data_s.length">
        </el-pagination>
        <!-- 添加的弹出表单 -->
        <el-dialog title="新增和编辑系统变量" :visible.sync="sys_var_dialog">
            ...
            <el-form :model="form_var_data" ref="var_data_from">
                <el-form-item label="变量名" prop="var_name">
                    <el-input v-model="form_var_data.var_name" v-if="isEditor" autocomplete="off" placeholder="变量名命名格式: #{变量名}" />
                    <el-input v-model="form_var_data.var_name" v-else disabled autocomplete="off" placeholder="变量名" />
                </el-form-item>
    ...
    methods: {
        ...
        delVarData(sys_var_id) {
            this.$confirm('确认删除吗?', '提示', {
                ...
                vcFun.deleteVariableConfigData({ "sys_var_id_s": sys_var_id }).then(res => {
                    if (res.success) {
